...
### Security hardening (Phase 16 — overnight hardening)

- **Auth-guard completeness test** — `__tests__/unit/auth/auth-guard-completeness.test.ts` walks every `route.ts` under `app/api/` and asserts that any non-whitelisted route returns 401 unauthenticated. The whitelist is held in the test itself with a written reason per entry; adding a new public route without justification fails CI.
- **Demo mode is a server-only concept** — `RouteMap` no longer reads `process.env.NEXT_PUBLIC_DEMO_MODE`. The browser bundle has no client-visible demo signal. Demo deploys leave `NEXT_PUBLIC_GOOGLE_MAPS_BROWSER_KEY` unset, which routes the component into the static placeholder; pages that already know runtime status (via `/api/demo/status`) can pass `forceStatic` explicitly.
- **Enquiry soft-delete** — `Enquiry.deletedAt` / `Enquiry.deletedById` close the gap left by Phase 15. The repository filters `deletedAt: null` on every read; `delete()` tombstones, `restore()` un-tombstones, `hardDelete()` is reserved for GDPR/FADP erasure.
