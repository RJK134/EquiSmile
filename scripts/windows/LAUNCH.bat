...
REM `next.config.ts` sets `output: 'standalone'` (for Docker), so
REM `next start` refuses to serve the build. Run the standalone
REM server directly after copying static assets into the bundle.
