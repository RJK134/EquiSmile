...
describe('googleMapsClient.optimizeRoute', () => {
  ...
  it('calls real Route Optimization API when DEMO_MODE=true and EQUISMILE_LIVE_MAPS=true', async () => {
    vi.stubEnv('GOOGLE_MAPS_API_KEY', 'test-api-key');
    vi.stubEnv('GCP_PROJECT_ID', 'test-project');
    vi.mocked(isDemoMode).mockReturnValue(true);
    vi.mocked(isLiveMapsForced).mockReturnValue(true);
    vi.mocked(routeOptimizerService.optimizeRoute).mockResolvedValue({
      visits: [
        {
          visitRequestId: 'stop-a',
    ...
    expect(routeOptimizerService.optimizeRoute).toHaveBeenCalled();
    expect(simulateRouteOptimization).not.toHaveBeenCalled();
    expect(result.mode).toBe('live');
  });

  it('uses simulator in demo mode when EQUISMILE_LIVE_MAPS is unset', async () => {
    vi.stubEnv('GOOGLE_MAPS_API_KEY', 'test-api-key');
    vi.stubEnv('GCP_PROJECT_ID', 'test-project');
    vi.mocked(isDemoMode).mockReturnValue(true);
    vi.mocked(isLiveMapsForced).mockReturnValue(false);
  ...
  it('still simulates when EQUISMILE_LIVE_MAPS=true but credentials are missing', async () => {
